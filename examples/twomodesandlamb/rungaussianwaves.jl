...
function runsimulation(Ro, ep, nkw, nx, plotname)
  ...
  @printf("\n
    *** %s ***, 
    Ro: %.2f, alph: %.3f, sig/f: %.1f, uw: %.2f m/s, nkw: %d\n\n",
    plotname, Ro, alph, sig/f0, uw, nkw
  )
  ...
  # Plot initialization
  #fig, axs4 = subplots(nrows=2, ncols=2, sharex=true, sharey=true,
  #  figsize=(6, 6))
  fig, axs2 = subplots(nrows=1, ncols=2, sharex=true, sharey=true,
    figsize=(12, 5))
  ...
  #makeplot!(axs, pl4)
  makeplot!(axs2, pl2)
  filename = @sprintf("%s/%s_%03d.png", plotpath, plotname, iplot)
